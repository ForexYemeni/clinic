...
import { Smartphone, X, Download, Sparkles, Share, PlusCircle } from 'lucide-react';
import { getPwaState, promptInstall, subscribe } from '@/lib/pwa-install';

// ─── Bottom Sheet Install Prompt (shown after beforeinstallprompt fires) ───
export function PwaInstallPrompt() {
  ...
  const [dismissed, setDismissed] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      if (dismissedAt) {
        ...
        if (Date.now() - dismissedTime < threeDaysMs) return true;
  ...
  useEffect(() => {
    const unsub = subscribe(() => setState(getPwaState()));
    setState(getPwaState());
    return unsub;
  }, []);

  useEffect(() => {
    // Show bottom sheet when deferred prompt becomes available
    if (state.deferredPrompt && !dismissed) {
  ...
  if (state.isStandalone || dismissed) return null;

  return (
    <AnimatePresence>
      {visible && state.deferredPrompt && (
        <motion.div
        ...
        >
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-border shadow-2xl shadow-black/15 dark:shadow-black/40 overflow-hidden">
            ...
            <button onClick={handleDismiss} className="absolute top-4 left-4 w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors z-10">
            ...
            <div className="p-5 pt-4">
              <div className="flex items-start gap-4 mb-4">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center flex-shrink-0 shadow-xl shadow-green-500/30 relative overflow-hidden">
                  ...
                  <div className="relative">
                    <img src="/icons/icon-192x192.png" alt="" className="w-10 h-10 rounded-xl" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              ...
              <div className="flex gap-2">
                <button onClick={handleInstall} className="flex-1 flex items-center justify-center gap-2 h-12 bg-gradient-to-l from-green-500 to-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-green-500/30 active:scale-95 transition-transform">
                  <Download className="w-5 h-5" />
                  تثبيت الآن
                </button>
                <button onClick={handleDismiss} className="h-12 px-4 bg-gray-100 dark:bg-gray-700 rounded-xl text-xs font-medium text-muted-foreground active:scale-95 transition-transform">
                  لاحقاً
                </button>
              </div>
            </div>
...
// Prominent card that shows on the dashboard itself
  ...
  const [dismissed, setDismissed] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      if (dismissedAt) {
        ...
        if (Date.now() - dismissedTime < sevenDaysMs) return true;
  ...
  return (
    ...
    >
      ...
      <div className="relative p-4">
        {/* Close button */}
        <button onClick={handleDismiss} className="absolute top-2 left-2 w-6 h-6 rounded-full bg-white/60 dark:bg-gray-700/60 flex items-center justify-center hover:bg-white dark:hover:bg-gray-600 transition-colors">
        ...
        {!showIOSGuide ? (
          <>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-green-500/25 relative overflow-hidden">
                ...
                <div className="relative flex items-center justify-center">
                  <img src="/icons/icon-192x192.png" alt="" className="w-9 h-9 rounded-xl" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            ...
            >
              <Download className="w-5 h-5" />
              {state.deferredPrompt ? 'تثبيت التطبيق الآن' : state.isIOS ? 'كيفية التثبيت على iPhone' : 'تثبيت التطبيق'}
            </button>

            {/* Hint text for when deferred prompt is not available */}
            {!state.deferredPrompt && !state.isIOS && (
              <p className="text-[10px] text-center text-muted-foreground mt-2">
                إذا لم يعمل الزر، استخدم خيار "تثبيت التطبيق" من قائمة المتصفح ⋮
              </p>
            )}
          </>
        ) : (
          ...
          <div className="space-y-3">
            ...
            <button onClick={handleDismiss} className="w-full h-10 bg-gray-100 dark:bg-gray-700 rounded-xl text-xs font-medium text-muted-foreground active:scale-[0.97] transition-transform">
              تم التثبيت / لاحقاً
            </button>
          </div>
        )}
