...
import { BarChart3, DollarSign, Users, Activity, Calendar, Clock, ArrowRight, FileText, CheckCircle, AlertTriangle } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        const result = await apiGet<ReportStats>(`/api/reports?type=${reportType}${nurseParam}`);
        setData(result);
      } catch (err) {
  ...
  return (
    <div className="p-4 pb-24">
      ...
      <div className="flex items-center gap-3 mb-4">
        <button onClick={() => setScreen('nurse-dashboard')} className="flex items-center gap-1 text-sm text-muted-foreground">
