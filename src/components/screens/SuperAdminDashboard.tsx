...
interface Props {
  initialTab?: 'dashboard' | 'clinics' | 'add' | 'platform' | 'settings';
}
...
export function SuperAdminDashboard({ initialTab = 'dashboard' }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'clinics' | 'add' | 'platform' | 'settings'>(initialTab);
  ...
  const [mongoProjectId, setFirebaseProjectId] = useState('');
  const [mongoClientEmail, setFirebaseClientEmail] = useState('');
  const [mongoPrivateKey, setFirebasePrivateKey] = useState('');
  const [savingPlatform, setSavingFirebase] = useState(false);
  ...
  const handleSaveFirebase = async (e: React.FormEvent) => {
    ...
    try {
      await apiPut('/api/super-admin/platform-config', {
        projectId: mongoProjectId,
        clientEmail: mongoClientEmail,
        privateKey: mongoPrivateKey,
      });
      toast.success('تم تحديث إعدادات المنصة');
    } catch (err: any) {
      toast.error(err.message || 'خطأ في تحديث الإعدادات');
    } finally {
    ...
    return (
      <div className="p-4 space-y-4 pb-20">
        ...
        <div className="space-y-2">
          ...
          <button onClick={() => setScreen('super-admin-platform-config')} className="w-full flex items-center gap-3 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-xl text-orange-700 dark:text-orange-300 touch-feedback">
            <Database className="w-5 h-5" />
            <div className="text-right">
              <p className="text-sm font-bold">إعدادات المنصة</p>
              <p className="text-xs opacity-70">إعدادات قاعدة البيانات والمنصة</p>
            </div>
          </button>
        </div>
      ...
      <div className="p-4 space-y-4 pb-20">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">إعدادات قاعدة البيانات والمنصة</p>
          </div>
        </div>

        <div className="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-xl">
          <p className="text-xs text-orange-700 dark:text-orange-300">
            تحذير: تغيير إعدادات المنصة سيؤثر على اتصال التطبيق بقاعدة البيانات. تأكد من صحة البيانات قبل الحفظ.
          </p>
        </div>
        ...
        <form onSubmit={handleSaveFirebase} className="space-y-4">
          <div className="space-y-2">
            ...
            <input type="text" value={mongoProjectId} onChange={(e) => setFirebaseProjectId(e.target.value)} placeholder="اسم المشروع"
          ...
          <div className="space-y-2">
            ...
            <input type="email" value={mongoClientEmail} onChange={(e) => setFirebaseClientEmail(e.target.value)} placeholder="بريد المشرف"
              className="w-full h-12 px-4 bg-white dark:bg-gray-800 border border-border rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-purple-500" dir="ltr" />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Private Key</label>
            <textarea value={mongoPrivateKey} onChange={(e) => setFirebasePrivateKey(e.target.value)} placeholder="مفتاح الاتصال (اختياري)"
          ...
          <button type="submit" disabled={savingPlatform} className="w-full h-12 bg-gradient-to-l from-orange-500 to-orange-600 text-white font-bold rounded-xl shadow-lg disabled:opacity-60 active:scale-[0.98] transition-all">
            {savingPlatform ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto" /> : 'حفظ إعدادات المنصة'}
          </button>
        </form>
      </div>
    );
      ...
      <>
        <div className="p-4 space-y-4 pb-20">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-border overflow-hidden">
            ...
            <button onClick={() => setScreen('super-admin-platform-config')} className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              <div className="flex items-center gap-3">
                <Database className="w-5 h-5 text-orange-500" />
                <div className="text-right">
                  <p className="text-sm font-medium">إعدادات المنصة</p>
                  <p className="text-xs text-muted-foreground">إعدادات قاعدة البيانات والمنصة</p>
                </div>
              </div>
