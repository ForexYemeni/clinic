...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Handle logo separately - validate size (Firestore doc limit ~1MB)
    if (logo !== undefined) {
      if (logo && logo.length > 900000) {
        // Logo base64 too large for Firestore document, skip saving it
    ...
    const snapshot = await adminDb.collection('clinic').limit(1).get();
    if (snapshot.empty) {
      // No clinic document exists yet - create one
      const createData = {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete operational data in batches (Firestore limit: 500 ops per batch)
    ...
    for (let i = 0; i < DEFAULT_SERVICES.length; i += BATCH_LIMIT) {
      const batch = adminDb.batch();
      const chunk = DEFAULT_SERVICES.slice(i, i + BATCH_LIMIT);
      chunk.forEach(service => {
        const ref = adminDb.collection('services').doc();
        batch.set(ref, { ...service, createdAt: new Date().toISOString() });
      });
      await batch.commit();
    }
